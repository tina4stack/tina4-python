...
def test_health_omits_the_version_outside_debug(project_dir, monkeypatch):
    monkeypatch.setenv("TINA4_DEBUG", "false")
    for path in ("/health", "/__health"):
        status, body, _ = _dispatch("GET", path)
        assert status == 200
        assert set(_json.loads(body)) == {"status", "uptime", "framework"}


def test_health_carries_the_version_in_debug(project_dir):
    from tina4_python import __version__
    status, body, _ = _dispatch("GET", "/health")
    assert status == 200
    assert _json.loads(body)["version"] == __version__


# ── register() is not an ungated side door ──────────────────────────────────

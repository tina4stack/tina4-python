...
def _dev_allowed_host_names() -> set:
    """Host names the dev surface answers to (ADR-0082): loopback names plus the
...
def dev_host_allowed(headers) -> bool:
    ...
    configured TINA4_HOST (ADR-0082). A missing Host is not a browser request
...
def _dev_same_origin_ok(request) -> bool:
    ...
    if origin:
        ...
        try:
            ...
            scheme = "https" if request.is_secure_scheme() else "http"
            expected = urlsplit(f"{scheme}://{host}")
            return (bool(host) and given.scheme == expected.scheme
                    and given.hostname == expected.hostname
                    and (given.port or (443 if given.scheme == "https" else 80))
                    == (expected.port or (443 if expected.scheme == "https" else 80))
                    and not given.username and not given.password
                    and not given.query and not given.fragment
                    and given.path in ("", "/"))
        except ValueError:
            return False
    return True
...
def _dev_request_denial(request):
    ...
    One gate for EVERY /__dev request, reads and writes alike (ADR-0082):
...
def _dev_resolve_path(rel: str, roots=None):
    ...
    ``None`` when the result leaves every allowed root (ADR-0082). The root
...
def register():
    ...
    """
    # ADR-0082: the dev handlers are served ONLY by the gated /__dev dispatch
...
async def _api_metrics_file(request, response):
    ...
    # ADR-0082: the path must resolve inside the project or the tree the last
...
async def _api_files(request, response):
    ...
    # Security: must stay within project root (ADR-0082)
    if resolved is None:
...
async def _api_file_read(request, response):
    ...
    # judged on the RESOLVED path (ADR-0082).
...
def _mcp_token_ok(request, *, dedicated: bool = False) -> bool:
    """MCP transport retains API-key fallback; general dev routes require the
    dedicated MCP token and cannot use the application API key."""
    import os as _os, hmac as _hmac
    expected = (_os.environ.get("TINA4_MCP_TOKEN")
                or (None if dedicated else _os.environ.get("TINA4_API_KEY")) or "")
    if not expected:
        return False
    headers = getattr(request, "headers", None) or {}
    ...
    if not provided and not dedicated:

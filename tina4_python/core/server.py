...
def _resolve_template(path: str) -> str | None:
    ...
    if is_dev:
        ...
        if any(seg.startswith("_") for seg in clean_path.split("/")):
            return None
        # ADR-0082: a URL segment of "." or ".." (or a backslash) could walk out
...
def _dev_socket_host_ok(headers: dict) -> bool:
    """Host allow-list for the /__dev_reload socket (ADR-0082): the same rule
...
async def _handle_dev_websocket(reader, writer, headers, path, query_string: str = ""):
    ...
    # ADR-0082: the dev reload socket answers only a loopback / TINA4_HOST Host.

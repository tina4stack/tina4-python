...
class PostgreSQLAdapter(SqlCrudMixin, DatabaseAdapter):
    ...
    def fetch(self, sql: str, params: list = None,
        ...
        # #133: a write that returns rows (INSERT ... RETURNING) can be
        # neither COUNT-probed nor paginated - PostgreSQL rejects both, and a
        # probe that ran would perform the write twice. It runs once, as
        # written; _total_from_page treats the un-paginated page as the total.
        ...
        # For a write (is_write -> not paginated) the page IS the total, so no
        # probe runs (#133: a probe would repeat the write).

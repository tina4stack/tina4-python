...
`;function $t(e){e.innerHTML=`
  ...
  `).join(""))}window.__loadRoutes=Ve;let Q=[],U=[],j=JSON.parse(localStorage.getItem("tina4_query_history")||"[]");function kt(e){e.innerHTML=`
    <div class="dev-panel-header">
      <h2>Database</h2>
  ...
  `,$e(),Ee()}async function $e(){const t=(await T("/tables")).tables||[],n=document.getElementById("db-table-list");n&&(n.innerHTML=t.length?t.map(s=>`<div style="padding:0.3rem 0.5rem;cursor:pointer;border-radius:0.25rem;font-size:0.8rem;font-family:monospace" class="db-table-item" onclick="window.__selectTable('${r(s)}')" onmouseover="this.style.background='var(--border)'" onmouseout="this.style.background=''">${r(s)}</div>`).join(""):'<div class="text-sm text-muted">No tables</div>');const i=document.getElementById("db-seed-table");i&&(i.innerHTML='<option value="">Pick table...</option>'+t.map(s=>`<option value="${r(s)}">${r(s)}</option>`).join(""));const o=document.getElementById("paste-table");o&&(o.innerHTML='<option value="">Select table...</option>'+t.map(s=>`<option value="${r(s)}">${r(s)}</option>`).join(""))}function ke(e){var n;(n=document.getElementById("db-limit"))!=null&&n.value;const t=document.getElementById("db-query");t&&(t.value=`SELECT * FROM ${e}`),document.querySelectorAll(".db-table-item").forEach(i=>{i.style.background=i.textContent===e?"var(--border)":""}),Ye()}function Et(){var n;const e=document.getElementById("db-query"),t=((n=document.getElementById("db-limit"))==null?void 0:n.value)||"20";e!=null&&e.value&&(e.value=e.value.replace(/LIMIT\s+\d+/i,`LIMIT ${t}`))}function St(e){const t=e.trim();t&&(j=j.filter(n=>n!==t),j.unshift(t),j.length>50&&(j=j.slice(0,50)),localStorage.setItem("tina4_query_history",JSON.stringify(j)),Ee())}function Ee(){const e=document.getElementById("db-history");e&&(e.innerHTML='<option value="">Query history...</option>'+j.map((t,n)=>`<option value="${n}">${r(t.length>80?t.substring(0,80)+"...":t)}</option>`).join(""))}function Tt(e){const t=parseInt(e);if(isNaN(t)||!j[t])return;const n=document.getElementById("db-query");n&&(n.value=j[t]),document.getElementById("db-history").selectedIndex=0}function It(){j=[],localStorage.removeItem("tina4_query_history"),Ee()}async function Ye(){var o,s,d;const e=document.getElementById("db-query"),t=(o=e==null?void 0:e.value)==null?void 0:o.trim();if(!t)return;St(t);const n=document.getElementById("db-result"),i=((s=document.getElementById("db-type"))==null?void 0:s.value)||"sql";n&&(n.innerHTML='<p class="text-muted">Running...</p>');try{const a=parseInt(((d=document.getElementById("db-limit"))==null?void 0:d.value)||"20"),c=await T("/query","POST",{query:t,type:i,limit:a});if(c.error){n&&(n.innerHTML=`<p style="color:var(--danger)">${r(c.error)}</p>`);return}c.rows&&c.rows.length>0?(U=Object.keys(c.rows[0]),Q=c.rows,n&&(n.innerHTML=`<p class="text-sm text-muted" style="margin-bottom:0.5rem">${c.count??c.rows.length} rows</p>
        <div style="overflow-x:auto"><table><thead><tr>${U.map(l=>`<th>${r(l)}</th>`).join("")}</tr></thead>
        <tbody>${c.rows.map(l=>`<tr>${U.map(y=>`<td class="text-sm">${r(String(l[y]??""))}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`)):c.affected!==void 0?(n&&(n.innerHTML=`<p class="text-muted">${c.affected} rows affected. ${c.success?"Success.":""}</p>`),Q=[],U=[]):(n&&(n.innerHTML='<p class="text-muted">No results</p>'),Q=[],U=[])}catch(a){n&&(n.innerHTML=`<p style="color:var(--danger)">${r(a.message)}</p>`)}}function qt(){if(!Q.length)return;const e=U.join(","),t=Q.map(n=>U.map(i=>{const o=String(n[i]??"");return o.includes(",")||o.includes('"')?`"${o.replace(/"/g,'""')}"`:o}).join(","));navigator.clipboard.writeText([e,...t].join(`
`))}function Mt(){Q.length&&navigator.clipboard.writeText(JSON.stringify(Q,null,2))}function Lt(){const e=document.getElementById("db-paste-modal");e&&(e.style.display="flex")}function Ke(){const e=document.getElementById("db-paste-modal");e&&(e.style.display="none")}async function Ct(){var o,s,d,a,c;const e=(o=document.getElementById("paste-table"))==null?void 0:o.value,t=(d=(s=document.getElementById("paste-new-table"))==null?void 0:s.value)==null?void 0:d.trim(),n=t||e,i=(c=(a=document.getElementById("paste-data"))==null?void 0:a.value)==null?void 0:c.trim();if(!n||!i){alert("Select a table or enter a new table name, and paste data.");return}try{let l;try{l=JSON.parse(i),Array.isArray(l)||(l=[l])}catch{const _=i.split(`
`).map(E=>E.trim()).filter(Boolean);if(_.length<2){alert("CSV needs at least a header row and one data row.");return}const g=_[0].split(",").map(E=>E.trim().replace(/[^a-zA-Z0-9_]/g,""));l=_.slice(1).map(E=>{const S=E.split(",").map(H=>H.trim()),w={};return g.forEach((H,ie)=>{w[H]=S[ie]??""}),w})}if(!l.length){alert("No data rows found.");return}if(t){const g=["id INTEGER PRIMARY KEY AUTOINCREMENT",...Object.keys(l[0]).filter(S=>S.toLowerCase()!=="id").map(S=>`"${S}" TEXT`)],E=await T("/query","POST",{query:`CREATE TABLE IF NOT EXISTS "${t}" (${g.join(", ")})`,type:"sql"});if(E.error){alert("Create table failed: "+E.error);return}}let y=0;for(const _ of l){const g=t?Object.keys(_).filter(H=>H.toLowerCase()!=="id"):Object.keys(_),E=g.map(H=>`"${H}"`).join(","),S=g.map(H=>`'${String(_[H]).replace(/'/g,"''")}'`).join(","),w=await T("/query","POST",{query:`INSERT INTO "${n}" (${E}) VALUES (${S})`,type:"sql"});if(w.error){alert(`Row ${y+1} failed: ${w.error}`);break}y++}document.getElementById("paste-data").value="",document.getElementById("paste-new-table").value="",document.getElementById("paste-table").selectedIndex=0,Ke(),$e(),y>0&&ke(n)}catch(l){alert("Import error: "+l.message)}}async function Bt(){var n,i;const e=(n=document.getElementById("db-seed-table"))==null?void 0:n.value,t=parseInt(((i=document.getElementById("db-seed-count"))==null?void 0:i.value)||"10");if(e)try{const o=await T("/seed","POST",{table:e,count:t});o.error?alert(o.error):ke(e)}catch(o){alert("Seed error: "+o.message)}}window.__loadTables=$e,window.__selectTable=ke,window.__updateLimit=Et,window.__runQuery=Ye,window.__copyCSV=qt,window.__copyJSON=Mt,window.__showPaste=Lt,window.__hidePaste=Ke,window.__doPaste=Ct,window.__seedTable=Bt,window.__loadHistory=Tt,window.__clearHistory=It;function zt(e){e.innerHTML=`
    <div class="dev-panel-header">
    ...
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:0.5rem;padding:0.75rem;margin-bottom:0.75rem">
      <div class="flex items-center" style="justify-content:space-between;flex-wrap:wrap;gap:0.5rem">
        <div style="flex:1;min-width:0">
          ...
          <strong style="margin-left:0.5rem;font-size:0.85rem">${r(d)}</strong>
        </div>
...
${n.traceback||""}`;window.__switchTab("chat"),setTimeout(()=>{window.__prefillChat(d)},150)}window.__loadErrors=le,window.__clearErrors=Rt,window.__resolveError=Ht,window.__askAboutError=At;function jt(e){e.innerHTML=`
  ...
  `).join(""),n){const a=[];e.debug!==void 0&&a.push(["TINA4_DEBUG",String(e.debug)]),e.log_level&&a.push(["LOG_LEVEL",e.log_level]),e.database&&a.push(["DATABASE_URL",e.database]),a.length&&(n.innerHTML=`
        ...
        <table>
          ...
          <tbody>${a.map(([c,l])=>`<tr><td class="text-mono text-sm" style="padding:4px 8px">${r(c)}</td><td class="text-sm" style="padding:4px 8px">${r(l)}</td></tr>`).join("")}</tbody>
        </table>
  ...
  `,Ft()}async function Ft(){var s;const e=document.getElementById("metrics-chart"),t=document.getElementById("metrics-complex"),n=document.getElementById("metrics-scan-info");e&&(e.style.display="block",e.innerHTML='<p class="text-muted">Analyzing...</p>');const i=await T("/metrics/full");if(i.error||!i.file_metrics){e&&(e.innerHTML=`<p style="color:var(--danger)">${r(i.error||"No data")}</p>`);return}if(n){const d=i.scan_mode==="framework"?'<span style="color:#cba6f7;font-weight:600">(Framework)</span> Add code to src/ to see your project':"";n.innerHTML=`${i.files_analyzed} files analyzed | ${i.total_functions} functions ${d}`}const o=document.getElementById("metrics-quick");o&&(o.innerHTML=[F("Files Analyzed",i.files_analyzed),F("Total Functions",i.total_functions),F("Avg Complexity",i.avg_complexity),F("Avg Maintainability",i.avg_maintainability)].join("")),e&&i.file_metrics.length>0?Dt(i.file_metrics,e,i.dependency_graph||{},i.scan_mode||"project"):e&&(e.innerHTML='<p class="text-muted">No files to visualize</p>'),t&&((s=i.most_complex_functions)!=null&&s.length)&&(t.innerHTML=`
      ...
      <table>
        ...
        <tbody>${i.most_complex_functions.slice(0,15).map(d=>`
          <tr>
            <td class="text-mono">${r(d.name)}</td>
            <td class="text-sm text-muted" style="cursor:pointer;text-decoration:underline dotted" onclick="window.__drillDown('${r(d.file)}')">${r(d.file)}</td>
            <td>${d.line}</td>
    ...
    `)}function Dt(e,t,n,i){var bt,ht,ft;const o=t.offsetWidth||900,s=Math.max(450,Math.min(650,o*.45)),d=Math.max(...e.map(f=>f.loc))||1,a=Math.max(...e.map(f=>f.dep_count||0))||1,c=14,l=Math.min(70,o/10);function y(f){const b=Math.min((f.avg_complexity||0)/10,1),v=f.has_tests?0:1,$=Math.min((f.dep_count||0)/5,1),p=b*.4+v*.4+$*.2,m=Math.max(0,Math.min(1,p)),h=Math.round(120*(1-m)),x=Math.round(70+m*30),k=Math.round(42+18*(1-m));return`hsl(${h},${x}%,${k}%)`}function _(f){return f.loc/d*.4+(f.avg_complexity||0)/10*.4+(f.dep_count||0)/a*.2}const g=[...e].sort((f,b)=>_(f)-_(b)),E=o/2,S=s/2,w=[];let H=0,ie=0;for(const f of g){const b=c+Math.sqrt(_(f))*(l-c),v=y(f);let $=!1;for(let p=0;p<800;p++){const m=E+ie*Math.cos(H),h=S+ie*Math.sin(H);let x=!1;for(const k of w){const C=m-k.x,z=h-k.y;if(Math.sqrt(C*C+z*z)<b+k.r+2){x=!0;break}}if(!x&&m>b+2&&m<o-b-2&&h>b+25&&h<s-b-2){w.push({x:m,y:h,vx:0,vy:0,r:b,color:v,f}),$=!0;break}H+=.2,ie+=.04}$||w.push({x:E+(Math.random()-.5)*o*.3,y:S+(Math.random()-.5)*s*.3,vx:0,vy:0,r:b,color:v,f})}const Je=[];function lt(f){const b=f.replace(/\\/g,"/").split("/").pop()||"",v=b.lastIndexOf(".");return(v>0?b.substring(0,v):b).toLowerCase()}const We={};w.forEach((f,b)=>{We[lt(f.f.path)]=b});for(const[f,b]of Object.entries(n)){let v=null;if(w.forEach(($,p)=>{$.f.path===f&&(v=p)}),v!==null)for(const $ of b){const p=$.replace(/^\.\//,"").replace(/^\.\.\//,"").split(/[./]/);let m;for(let h=p.length-1;h>=0;h--){const x=p[h].toLowerCase();if(x&&x!=="js"&&x!=="py"&&x!=="rb"&&x!=="ts"&&x!=="index"&&(m=We[x],m!==void 0))break}m===void 0&&(m=We[lt($)]),m!==void 0&&v!==m&&Je.push([v,m])}}const q=document.createElement("canvas");q.width=o,q.height=s,q.style.cssText="display:block;border:1px solid var(--border);border-radius:8px;cursor:pointer;background:#0f172a";const En=i==="framework"?'<span style="color:#cba6f7;font-weight:600">(Framework)</span> Add code to src/ to see your project':"";t.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.5rem"><h3 style="margin:0;font-size:0.85rem">Code Landscape ${En}</h3><span style="font-size:0.65rem;color:var(--muted)">Drag bubbles | Dbl-click to drill down</span></div><div style="position:relative" id="metrics-canvas-wrap"></div>`,document.getElementById("metrics-canvas-wrap").appendChild(q);const Qe=document.createElement("div");Qe.style.cssText="position:absolute;top:8px;left:8px;z-index:2;display:flex;gap:4px;flex-direction:column",Qe.innerHTML=`
  ...
  `,document.getElementById("metrics-canvas-wrap").appendChild(Qe),(bt=document.getElementById("metrics-zoom-in"))==null||bt.addEventListener("click",()=>{L=Math.min(5,L*1.3)}),(ht=document.getElementById("metrics-zoom-out"))==null||ht.addEventListener("click",()=>{L=Math.max(.3,L*.7)}),(ft=document.getElementById("metrics-zoom-fit"))==null||ft.addEventListener("click",()=>{L=1,K=0,X=0});const u=q.getContext("2d");let G=-1,M=-1,dt=0,ct=0,K=0,X=0,L=1,se=!1,mt=0,ut=0,pt=0,gt=0;function Sn(){for(let p=0;p<w.length;p++){if(p===M)continue;const m=w[p],h=E-m.x,x=S-m.y,k=.3+m.r/l*.7,C=.008*k*k;m.vx+=h*C,m.vy+=x*C}for(const[p,m]of Je){const h=w[p],x=w[m],k=x.x-h.x,C=x.y-h.y,z=Math.sqrt(k*k+C*C)||1,P=h.r+x.r+20,N=(z-P)*.002,re=k/z*N,ae=C/z*N;p!==M&&(h.vx+=re,h.vy+=ae),m!==M&&(x.vx-=re,x.vy-=ae)}for(let p=0;p<w.length;p++)for(let m=p+1;m<w.length;m++){const h=w[p],x=w[m],k=x.x-h.x,C=x.y-h.y,z=Math.sqrt(k*k+C*C)||1,P=h.r+x.r+20;if(z<P){const N=40*(P-z)/P,re=k/z*N,ae=C/z*N;p!==M&&(h.vx-=re,h.vy-=ae),m!==M&&(x.vx+=re,x.vy+=ae)}}for(let p=0;p<w.length;p++){if(p===M)continue;const m=w[p];m.vx*=.65,m.vy*=.65;const h=2;m.vx=Math.max(-h,Math.min(h,m.vx)),m.vy=Math.max(-h,Math.min(h,m.vy)),m.x+=m.vx,m.y+=m.vy,m.x=Math.max(m.r+2,Math.min(o-m.r-2,m.x)),m.y=Math.max(m.r+25,Math.min(s-m.r-2,m.y))}}function yt(){var f;Sn(),u.clearRect(0,0,o,s),u.save(),u.translate(K,X),u.scale(L,L),u.strokeStyle="rgba(255,255,255,0.03)",u.lineWidth=1/L;for(let b=0;b<o/L;b+=50)u.beginPath(),u.moveTo(b,0),u.lineTo(b,s/L),u.stroke();for(let b=0;b<s/L;b+=50)u.beginPath(),u.moveTo(0,b),u.lineTo(o/L,b),u.stroke();for(const[b,v]of Je){const $=w[b],p=w[v],m=p.x-$.x,h=p.y-$.y,x=Math.sqrt(m*m+h*h)||1,k=G===b||G===v;u.beginPath(),u.moveTo($.x+m/x*$.r,$.y+h/x*$.r);const C=p.x-m/x*p.r,z=p.y-h/x*p.r;u.lineTo(C,z),u.strokeStyle=k?"rgba(139,180,250,0.9)":"rgba(255,255,255,0.15)",u.lineWidth=k?3:1,u.stroke();const P=k?12:6,N=Math.atan2(h,m);u.beginPath(),u.moveTo(C,z),u.lineTo(C-P*Math.cos(N-.4),z-P*Math.sin(N-.4)),u.lineTo(C-P*Math.cos(N+.4),z-P*Math.sin(N+.4)),u.closePath(),u.fillStyle=u.strokeStyle,u.fill()}for(let b=0;b<w.length;b++){const v=w[b],$=b===G,p=$?v.r+4:v.r;$&&(u.beginPath(),u.arc(v.x,v.y,p+8,0,Math.PI*2),u.fillStyle="rgba(255,255,255,0.08)",u.fill()),u.beginPath(),u.arc(v.x,v.y,p,0,Math.PI*2),u.fillStyle=v.color,u.globalAlpha=$?1:.85,u.fill(),u.globalAlpha=1,u.strokeStyle=$?"rgba(255,255,255,0.6)":"rgba(255,255,255,0.25)",u.lineWidth=$?2.5:1.5,u.stroke();const m=((f=v.f.path.split("/").pop())==null?void 0:f.replace(/\.\w+$/,""))||"?";if(p>16){const k=Math.max(8,Math.min(13,p*.38));u.fillStyle="#fff",u.font=`600 ${k}px monospace`,u.textAlign="center",u.fillText(m,v.x,v.y-2),u.fillStyle="rgba(255,255,255,0.65)",u.font=`${k-1}px monospace`,u.fillText(`${v.f.loc} LOC`,v.x,v.y+k)}const h=Math.max(9,p*.3),x=h*.7;if(p>14&&v.f.dep_count>0){const k=v.y-p+x+3;u.beginPath(),u.arc(v.x,k,x,0,Math.PI*2),u.fillStyle="#ea580c",u.fill(),u.fillStyle="#fff",u.font=`bold ${h}px sans-serif`,u.textAlign="center",u.fillText("D",v.x,k+h*.35)}if(p>14&&v.f.has_tests){const k=v.y+p-x-3;u.beginPath(),u.arc(v.x,k,x,0,Math.PI*2),u.fillStyle="#16a34a",u.fill(),u.fillStyle="#fff",u.font=`bold ${h}px sans-serif`,u.textAlign="center",u.fillText("T",v.x,k+h*.35)}}u.restore(),requestAnimationFrame(yt)}q.addEventListener("mousemove",f=>{const b=q.getBoundingClientRect(),v=(f.clientX-b.left-K)/L,$=(f.clientY-b.top-X)/L;if(se){K=pt+(f.clientX-mt),X=gt+(f.clientY-ut);return}if(M>=0){_e=!0,w[M].x=v+dt,w[M].y=$+ct,w[M].vx=0,w[M].vy=0;return}G=-1;for(let p=w.length-1;p>=0;p--){const m=w[p],h=v-m.x,x=$-m.y;if(Math.sqrt(h*h+x*x)<m.r+4){G=p;break}}q.style.cursor=G>=0?"grab":"default"}),q.addEventListener("mousedown",f=>{const b=q.getBoundingClientRect(),v=(f.clientX-b.left-K)/L,$=(f.clientY-b.top-X)/L;if(f.button===2){se=!0,mt=f.clientX,ut=f.clientY,pt=K,gt=X,q.style.cursor="move";return}G>=0&&(M=G,dt=w[M].x-v,ct=w[M].y-$,_e=!1,q.style.cursor="grabbing")});let _e=!1;q.addEventListener("mouseup",f=>{if(se){se=!1,q.style.cursor="default";return}if(M>=0){_e||Se(w[M].f.path),q.style.cursor="grab",M=-1,_e=!1;return}}),q.addEventListener("mouseleave",()=>{G=-1,M=-1,se=!1}),q.addEventListener("dblclick",f=>{const b=q.getBoundingClientRect(),v=(f.clientX-b.left-K)/L,$=(f.clientY-b.top-X)/L;for(let p=w.length-1;p>=0;p--){const m=w[p],h=v-m.x,x=$-m.y;if(Math.sqrt(h*h+x*x)<m.r+4){Se(m.f.path);break}}}),q.addEventListener("contextmenu",f=>f.preventDefault()),requestAnimationFrame(yt)}async function Se(e){const t=document.getElementById("metrics-detail");if(!t)return;t.innerHTML='<p class="text-muted">Loading file analysis...</p>';const n=await T("/metrics/file?path="+encodeURIComponent(e));if(n.error){t.innerHTML=`<p style="color:var(--danger)">${r(n.error)}</p>`;return}const i=n.functions||[],o=Math.max(1,...i.map(s=>s.complexity));t.innerHTML=`
  ...
  `,Ze()}async function Ze(){const e=document.getElementById("gql-types"),t=document.getElementById("gql-queries"),n=document.getElementById("gql-mutations");try{const i=await T("/graphql/schema");if(i.error){e&&(e.innerHTML=`<p class="text-sm" style="color:var(--danger)">${r(i.error)}</p>`);return}const o=i.schema||{},s=o.types||{},d=o.queries||{},a=o.mutations||{};if(e){const c=Object.keys(s);c.length?e.innerHTML=c.map(l=>{const y=s[l],_=Object.entries(y).map(([g,E])=>`<div style="padding-left:1rem;color:var(--muted);font-size:0.7rem">${r(g)}: <span style="color:var(--primary)">${r(String(E))}</span></div>`).join("");return`
            ...
            </div>`}).join(""):e.innerHTML='<p class="text-sm text-muted">No types registered</p>'}if(t){const c=Object.keys(d);c.length&&(t.innerHTML='<div style="font-weight:600;font-size:0.75rem;color:var(--muted);text-transform:uppercase;margin-bottom:0.25rem">Queries</div>'+c.map(l=>{const y=d[l],_=y.args?Object.entries(y.args).map(([g,E])=>`${g}: ${E}`).join(", "):"";return`<div style="font-size:0.8rem;padding:0.15rem 0;cursor:pointer;color:var(--text)" onclick="window.__insertGqlQuery('${r(l)}','query')" title="Click to insert">${r(l)}${_?`(${r(_)})`:""}: <span style="color:var(--primary)">${r(y.type||"")}</span></div>`}).join(""))}if(n){const c=Object.keys(a);c.length&&(n.innerHTML='<div style="font-weight:600;font-size:0.75rem;color:var(--muted);text-transform:uppercase;margin-bottom:0.25rem">Mutations</div>'+c.map(l=>{const y=a[l],_=y.args?Object.entries(y.args).map(([g,E])=>`${g}: ${E}`).join(", "):"";return`<div style="font-size:0.8rem;padding:0.15rem 0;cursor:pointer;color:var(--text)" onclick="window.__insertGqlQuery('${r(l)}','mutation')" title="Click to insert">${r(l)}${_?`(${r(_)})`:""}: <span style="color:var(--primary)">${r(y.type||"")}</span></div>`}).join(""))}}catch(i){e&&(e.innerHTML=`<p class="text-sm" style="color:var(--danger)">${r(i.message)}</p>`)}}function Jt(e,t){const n=document.getElementById("gql-query");n&&(t==="mutation"?n.value=`mutation {
  ${e} {
...
`);n&&(n.style.display="block",n.textContent=y)}i&&(i.textContent=JSON.stringify(l.data??l,null,2))}catch(l){n&&(n.style.display="block",n.textContent=l.message),i&&(i.textContent="")}}function Qt(){Te&&navigator.clipboard.writeText(JSON.stringify(Te,null,2))}window.__loadGqlSchema=Ze,window.__runGqlQuery=Wt,window.__copyGqlResult=Qt,window.__insertGqlQuery=Jt;let Ie=!1,qe=null,de="jobs",V="",R="default",ce=["default"],me=null;function Ut(e){me=e,T("/queue/topics").then(t=>{t.topics&&t.topics.length>0&&(ce=t.topics,ce.includes(R)||(R=ce[0])),Me()}).catch(()=>Me())}function Me(){if(!me)return;const e=ce.map(t=>`<option value="${r(t)}" ${t===R?"selected":""}>${r(t)}</option>`).join("");me.innerHTML=`
  ...
  `,J()}async function J(){if(de==="dead-letters"){Vt();return}try{const e=`?topic=${encodeURIComponent(R)}${V?`&status=${V}`:""}`,t=await T(`/queue${e}`),n=document.getElementById("queue-stats");if(n&&t.stats){const s=t.stats,d=(s.pending||0)+(s.reserved||0)+(s.completed||0)+(s.failed||0);n.innerHTML=`
        <span class="badge" style="background:var(--surface);border:1px solid var(--border);color:var(--text);cursor:pointer" onclick="window.__queueFilter('')">All: ${d}</span>
        ...
        <tbody>
          ${t.jobs.map(s=>`
            <tr>
              ...
              <td><code style="font-size:0.7rem;word-break:break-all;max-width:300px;display:inline-block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer" title="${r(JSON.stringify(s.data||{},null,2))}" onclick="window.__queueExpandPayload(this)">${r(JSON.stringify(s.data||{}).slice(0,80))}</code></td>
        ...
        <tbody>
          ${e.jobs.map(o=>`
            <tr>
              ...
              <td><code style="font-size:0.7rem;word-break:break-all;max-width:250px;display:inline-block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer" title="${r(JSON.stringify(o.data||{},null,2))}" onclick="window.__queueExpandPayload(this)">${r(JSON.stringify(o.data||{}).slice(0,60))}</code></td>
  ...
  `}let ye=0;function B(e,t){var s,d;const n=document.getElementById("chat-messages");if(!n)return;const i=`msg-${++ye}`,o=document.createElement("div");if(o.className=`chat-msg chat-${t}`,o.id=i,o.innerHTML=`
  ...
  `,o.addEventListener("mouseenter",()=>{const a=o.querySelector(".chat-msg-actions");a&&(a.style.opacity="1")}),o.addEventListener("mouseleave",()=>{const a=o.querySelector(".chat-msg-actions");a&&(a.style.opacity="0.4")}),o.querySelector(".chat-answer-input")){const a=o.querySelector("[data-submit-btn]");a&&(a.style.display="inline-block")}if(t==="bot"){const c=(((s=o.querySelector(".chat-msg-content"))==null?void 0:s.textContent)||"").trim().endsWith("?"),l=o.querySelector(".chat-answer-input");if(c&&!l){const y=document.createElement("div");y.style.cssText="display:flex;gap:4px;margin-top:6px;flex-wrap:wrap",y.className="chat-quick-replies",y.innerHTML=`
      ...
      `,(d=o.querySelector(".chat-msg-content"))==null||d.appendChild(y)}}n.prepend(o),Zt()}function rn(e){const t=document.getElementById(e);if(!t)return;const n=t.querySelectorAll(".chat-answer-input"),i=[];if(n.forEach(d=>{const a=d.dataset.q||"?",c=d.value.trim();c&&(i.push(`${a}. ${c}`),d.disabled=!0,d.style.opacity="0.6")}),!i.length)return;const o=document.getElementById("chat-input");o&&(o.value=i.join(`
...
`,i.focus(),i.setSelectionRange(i.value.length,i.value.length))}function cn(e){var i,o;const t=e.closest(".chat-checklist-item");if(!t||(i=t.nextElementSibling)!=null&&i.classList.contains("chat-comment-box"))return;const n=document.createElement("div");n.className="chat-comment-box",n.style.cssText="padding-left:1.8rem;margin:0.15rem 0;display:flex;gap:4px",n.innerHTML=`
  ...
  `,t.after(n),(o=n.querySelector("input"))==null||o.focus()}function mn(e){var s;const t=e.closest(".chat-comment-box");if(!t)return;const n=t.querySelector("input"),i=(s=n==null?void 0:n.value)==null?void 0:s.trim();if(!i)return;const o=document.createElement("div");o.style.cssText="padding-left:1.8rem;margin:0.1rem 0;font-size:0.7rem;color:var(--info);font-style:italic",o.textContent=`↳ ${i}`,t.replaceWith(o)}function et(){const e=[],t=[],n=[];return document.querySelectorAll(".chat-checklist-item").forEach(i=>{var a,c;const o=i.querySelector("input[type=checkbox]"),s=((a=i.querySelector("label"))==null?void 0:a.textContent)||"";o!=null&&o.checked?e.push(s):t.push(s);const d=i.nextElementSibling;if(d&&!d.classList.contains("chat-checklist-item")&&!d.classList.contains("chat-comment-box")){const l=((c=d.textContent)==null?void 0:c.replace("↳ ",""))||"";l&&n.push(`${s}: ${l}`)}}),{accepted:e,rejected:t,comments:n}}let be=!1;function je(){const e=document.getElementById("chat-thoughts-panel");e&&(be=!be,e.style.display=be?"block":"none",be&&tt())}async function tt(){const e=document.getElementById("thoughts-list");if(e)try{const i=(await(await fetch("/__dev/api/thoughts")).json()||[]).filter(s=>!s.dismissed),o=document.getElementById("thoughts-dot");if(o&&(o.style.display=i.length?"inline":"none"),!i.length){e.innerHTML='<div class="text-muted text-sm" style="text-align:center;padding:2rem 0">All clear. No observations.</div>';return}e.innerHTML=i.map(s=>`
      <div style="background:var(--bg);border:1px solid var(--border);border-radius:0.375rem;padding:0.5rem;margin-bottom:0.5rem;font-size:0.75rem">
        ...
        <div style="display:flex;gap:4px;margin-top:0.375rem">
          ${(s.actions||[]).map(d=>d.action==="dismiss"?`<button class="btn btn-sm" style="font-size:0.6rem" onclick="window.__dismissThought('${r(s.id)}')">Dismiss</button>`:`<button class="btn btn-sm btn-primary" style="font-size:0.6rem" onclick="window.__actOnThought('${r(s.id)}','${r(d.action)}')">${r(d.label)}</button>`).join("")}
        </div>
      </div>
    `).join("")}catch{e.innerHTML='<div class="text-muted text-sm" style="text-align:center;padding:1rem">Agent not connected</div>'}}async function nt(e){await fetch("/__dev/api/thoughts/dismiss",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:e})}).catch(()=>{}),tt()}function un(e,t){nt(e),je()}setInterval(async()=>{try{const n=(await(await fetch("/__dev/api/thoughts")).json()||[]).filter(o=>!o.dismissed),i=document.getElementById("thoughts-dot");i&&(i.style.display=n.length?"inline":"none")}catch{}},6e4),window.__dismissThought=nt,window.__actOnThought=un,window.__commentOnItem=cn,window.__submitComment=mn,window.__getChecklist=et;function pn(e){document.querySelectorAll(".chat-quick-replies").forEach(n=>n.remove());const t=document.getElementById("chat-input");t&&(t.value=e,Z())}window.__quickReply=pn,window.__copyMsg=ln,window.__replyMsg=dn,window.__clearChat=tn;const ee=[];function he(e){const t=document.getElementById("chat-status-bar"),n=document.getElementById("chat-status-text");t&&(t.style.display="flex"),n&&(n.textContent=e)}function ot(){const e=document.getElementById("chat-status-bar");e&&(e.style.display="none")}function fe(e,t){const n=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"});ee.unshift({time:n,text:e,agent:t}),ee.length>50&&(ee.length=50),Y()}async function Z(){var i;const e=document.getElementById("chat-input"),t=(i=e==null?void 0:e.value)==null?void 0:i.trim();if(!t)return;if(e.value="",B(r(t),"user"),D.length){const o=D.map(s=>s.name).join(", ");B(`<span class="text-sm text-muted">Attached: ${r(o)}</span>`,"user")}A="Thinking...",he("Analyzing request..."),fe("Analyzing request...","supervisor");const n={message:t,settings:{thinking:I.thinking,vision:I.vision,imageGen:I.imageGen}};D.length&&(n.files=D.map(o=>({name:o.name,data:o.data})));try{const o=await fetch("/__dev/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!o.ok||!o.body){const c=o.status===0?"Agent not running. Start: tina4 agent":`Error: ${o.status}`;B(`<span style="color:var(--danger)">${c}</span>`,"bot"),A="Error",Y();return}const s=o.body.getReader(),d=new TextDecoder;let a="";for(;;){const{done:c,value:l}=await s.read();if(c)break;a+=d.decode(l,{stream:!0});const y=a.split(`
`);a=y.pop()||"";let _="";for(const g of y)if(g.startsWith("event: "))_=g.slice(7).trim();else if(g.startsWith("data: ")){const E=g.slice(6);try{const S=JSON.parse(E);Oe(_,S)}catch{}}}D.length=0,Pe()}catch{B('<span style="color:var(--danger)">Connection failed</span>',"bot"),A="Error",Y()}}function Oe(e,t){switch(e){case"status":A=t.text||"Working...",he(`${t.agent||"supervisor"}: ${t.text||"Working..."}`),fe(t.text||"",t.agent||"supervisor");break;case"message":{const n=t.content||"",i=t.agent||"supervisor";let o=it(n);i!=="supervisor"&&(o=`<span class="badge" style="font-size:0.6rem;margin-right:4px">${r(i)}</span>`+o),t.files_changed&&t.files_changed.length>0&&(o+='<div style="margin-top:0.5rem;padding:0.5rem;background:var(--bg);border-radius:0.375rem;border:1px solid var(--border)">',o+='<div class="text-sm" style="color:var(--success);font-weight:600;margin-bottom:0.25rem">Files changed:</div>',t.files_changed.forEach(s=>{o+=`<div class="text-sm text-mono">${r(s)}</div>`,ge.includes(s)||ge.push(s)}),o+="</div>"),B(o,"bot");break}case"plan":{let n="";t.content&&(n+=it(t.content)),t.approve&&(n+=`
      ...
      `;B(s,"bot");break}case"done":A="Done",ot(),fe("Done","supervisor"),setTimeout(()=>{A="Idle",Y()},3e3);break}}async function gn(e){B(`<span style="color:var(--success)">Plan approved — let's build it!</span>`,"user"),A="Executing plan...",fe("Plan approved — building...","supervisor"),he("Building...");const t={plan_file:e,settings:{thinking:I.thinking,vision:I.vision,imageGen:I.imageGen}};try{const n=await fetch("/__dev/api/execute",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!n.ok||!n.body)return;const i=n.body.getReader(),o=new TextDecoder;let s="";for(;;){const{done:d,value:a}=await i.read();if(d)break;s+=o.decode(a,{stream:!0});const c=s.split(`
`);s=c.pop()||"";let l="";for(const y of c)if(y.startsWith("event: "))l=y.slice(7).trim();else if(y.startsWith("data: "))try{Oe(l,JSON.parse(y.slice(6)))}catch{}}}catch{B('<span style="color:var(--danger)">Plan execution failed</span>',"bot")}}function yn(e){B(`<span style="color:var(--muted)">Plan saved for later: ${r(e)}</span>`,"bot")}function bn(){const{accepted:e,rejected:t,comments:n}=et();let i=`Here's my feedback on the proposal:
...
`),!t.length&&!n.length&&(i+="Everything looks good. "),i+="Please revise the plan based on this feedback.";const o=document.getElementById("chat-input");o&&(o.value=i,Z())}async function hn(e){B('<span style="color:var(--info)">Resuming plan...</span>',"user"),A="Resuming...",he("Resuming...");const t={plan_file:e,resume:!0,settings:{thinking:I.thinking,vision:I.vision,imageGen:I.imageGen}};try{const n=await fetch("/__dev/api/execute",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!n.ok||!n.body)return;const i=n.body.getReader(),o=new TextDecoder;let s="";for(;;){const{done:d,value:a}=await i.read();if(d)break;s+=o.decode(a,{stream:!0});const c=s.split(`
`);s=c.pop()||"";let l="";for(const y of c)if(y.startsWith("event: "))l=y.slice(7).trim();else if(y.startsWith("data: "))try{Oe(l,JSON.parse(y.slice(6)))}catch{}}}catch{B('<span style="color:var(--danger)">Resume failed</span>',"bot")}}window.__resumePlan=hn,window.__submitFeedback=bn,window.__approvePlan=gn,window.__keepPlan=yn;async function fn(){try{const e=await T("/chat/undo","POST");B(`<span style="color:var(--warn)">${r(e.message||"Undo complete")}</span>`,"bot")}catch{B('<span style="color:var(--warn)">Nothing to undo</span>',"bot")}}const D=[];function vn(){const e=document.getElementById("chat-file-input");e!=null&&e.files&&(document.getElementById("chat-attachments"),Array.from(e.files).forEach(t=>{const n=new FileReader;n.onload=()=>{D.push({name:t.name,data:n.result}),Pe()},n.readAsDataURL(t)}),e.value="")}function Pe(){const e=document.getElementById("chat-attachments");if(e){if(!D.length){e.style.display="none";return}e.style.display="flex",e.style.cssText+="gap:0.375rem;flex-wrap:wrap;margin-bottom:0.375rem;font-size:0.75rem",e.innerHTML=D.map((t,n)=>`<span style="background:var(--surface);border:1px solid var(--border);border-radius:4px;padding:2px 8px;display:inline-flex;align-items:center;gap:4px">
      ${r(t.name)} <span style="cursor:pointer;color:var(--danger)" onclick="window.__removeFile(${n})">&times;</span>
    </span>`).join("")}}function xn(e){D.splice(e,1),Pe()}let te=!1,O=null;function wn(){const e=document.getElementById("chat-mic-btn"),t=window.SpeechRecognition||window.webkitSpeechRecognition;if(!t){B('<span style="color:var(--warn)">Speech recognition not supported in this browser</span>',"bot");return}if(te&&O){O.stop(),te=!1,e&&(e.textContent="Mic",e.style.background="");return}O=new t,O.continuous=!1,O.interimResults=!1,O.lang="en-US",O.onresult=n=>{const i=n.results[0][0].transcript,o=document.getElementById("chat-input");o&&(o.value=(o.value?o.value+" ":"")+i)},O.onend=()=>{te=!1,e&&(e.textContent="Mic",e.style.background="")},O.onerror=()=>{te=!1,e&&(e.textContent="Mic",e.style.background="")},O.start(),te=!0,e&&(e.textContent="Stop",e.style.background="var(--danger)")}window.__removeFile=xn;function it(e){let t=e.replace(/\\n/g,`
`);const n=[];t=t.replace(/```(\w*)\n([\s\S]*?)```/g,(d,a,c)=>{const l=n.length;return n.push(`<pre style="background:var(--bg);padding:0.75rem;border-radius:0.375rem;overflow-x:auto;margin:0.5rem 0;font-size:0.75rem;border:1px solid var(--border)"><code>${r(c)}</code></pre>`),`\0CODE${l}\0`});const i=t.split(`
`),o=[];for(const d of i){const a=d.trim();if(a.startsWith("\0CODE")){o.push(a);continue}if(a.startsWith("### ")){o.push(`<div style="font-weight:700;font-size:0.8rem;margin:0.75rem 0 0.25rem;color:var(--info)">${r(a.slice(4))}</div>`);continue}if(a.startsWith("## ")){o.push(`<div style="font-weight:700;font-size:0.9rem;margin:0.75rem 0 0.25rem">${r(a.slice(3))}</div>`);continue}if(a.startsWith("# ")){o.push(`<div style="font-weight:700;font-size:1rem;margin:0.75rem 0 0.25rem">${r(a.slice(2))}</div>`);continue}if(a==="---"||a==="***"){o.push('<hr style="border:none;border-top:1px solid var(--border);margin:0.5rem 0">');continue}const c=a.match(/^(\d+)[.)]\s+(.+)/);if(c){if(c[2].trim().endsWith("?")){const y=`q-${ye}-${c[1]}`;o.push(`<div style="margin:0.3rem 0;padding-left:0.5rem">
          ...
          <div style="display:flex;gap:4px;align-items:center;flex-wrap:wrap">
            <input type="text" class="input chat-answer-input" id="${y}" data-q="${c[1]}" placeholder="Your answer..." style="font-size:0.75rem;padding:4px 8px;flex:1;max-width:350px">
      ...
      </div>`);continue}if(a.startsWith("> ")){o.push(`<div style="border-left:3px solid var(--info);padding-left:0.75rem;margin:0.3rem 0;color:var(--muted);font-style:italic">${ne(a.slice(2))}</div>`);continue}if(a===""){o.push('<div style="height:0.4rem"></div>');continue}o.push(`<div style="margin:0.1rem 0">${ne(a)}</div>`)}let s=o.join("");return n.forEach((d,a)=>{s=s.replace(`\0CODE${a}\0`,d)}),s}function ne(e){return r(e).replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/\*(.+?)\*/g,"<em>$1</em>").replace(/`([^`]+)`/g,'<code style="background:var(--bg);padding:0.1rem 0.3rem;border-radius:0.2rem;font-size:0.8em;border:1px solid var(--border)">$1</code>')}function _n(e){const t=document.getElementById("chat-input");t&&(t.value=e,t.focus(),t.scrollTop=t.scrollHeight)}window.__sendChat=Z,window.__undoChat=fn,window.__prefillChat=_n;const st=document.createElement("style");st.textContent=_t,document.head.appendChild(st);const ve=xt();wt(ve);const Ne=[{id:"routes",label:"Routes",render:$t},{id:"database",label:"Database",render:kt},{id:"graphql",label:"GraphQL",render:Gt},{id:"queue",label:"Queue",render:Ut},{id:"errors",label:"Errors",render:zt},{id:"metrics",label:"Metrics",render:Nt},{id:"system",label:"System",render:jt}],rt={id:"chat",label:"Code With Me",render:nn};let xe=localStorage.getItem("tina4_cwm_unlocked")==="true",we=xe?[rt,...Ne]:[...Ne],oe=xe?"chat":"routes";function $n(){const e=document.getElementById("app");if(!e)return;e.innerHTML=`
    <div class="dev-admin">

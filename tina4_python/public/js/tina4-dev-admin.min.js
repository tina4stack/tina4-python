...
${n.traceback||""}`;window.__switchTab("chat"),setTimeout(()=>{window.__prefillChat(d)},150)}window.__loadErrors=le,window.__clearErrors=Rt,window.__resolveError=Ht,window.__askAboutError=At;function jt(e){e.innerHTML=`
  ...
  `,Ft()}async function Ft(){var s;const e=document.getElementById("metrics-chart"),t=document.getElementById("metrics-complex"),n=document.getElementById("metrics-scan-info");e&&(e.style.display="block",e.innerHTML='<p class="text-muted">Analyzing...</p>');const i=await T("/metrics/full");if(i.error||!i.file_metrics){e&&(e.innerHTML=`<p style="color:var(--danger)">${r(i.error||"No data")}</p>`);return}if(n){const d=i.scan_mode==="framework"?'<span style="color:#cba6f7;font-weight:600">(Framework)</span> Add code to src/ to see your project':"";n.innerHTML=`${i.files_analyzed} files analyzed | ${i.total_functions} functions ${d}`}const o=document.getElementById("metrics-quick");o&&(o.innerHTML=[F("Files Analyzed",i.files_analyzed),F("Total Functions",i.total_functions),F("Avg Complexity",i.avg_complexity),F("Avg Maintainability",i.avg_maintainability)].join("")),e&&i.file_metrics.length>0?Dt(i.file_metrics,e,i.dependency_graph||{},i.scan_mode||"project"):e&&(e.innerHTML='<p class="text-muted">No files to visualize</p>'),t&&((s=i.most_complex_functions)!=null&&s.length)&&(t.innerHTML=`
    ...
    `)}function Dt(e,t,n,i){var yt,ht,ft;const o=t.offsetWidth||900,s=Math.max(450,Math.min(650,o*.45)),d=Math.max(...e.map(f=>f.loc))||1,a=Math.max(...e.map(f=>f.dep_count||0))||1,c=14,l=Math.min(70,o/10);function b(f){const y=Math.min((f.avg_complexity||0)/10,1),v=f.has_tests?0:1,$=Math.min((f.dep_count||0)/5,1),p=y*.4+v*.4+$*.2,m=Math.max(0,Math.min(1,p)),h=Math.round(120*(1-m)),x=Math.round(70+m*30),k=Math.round(42+18*(1-m));return`hsl(${h},${x}%,${k}%)`}function _(f){return f.loc/d*.4+(f.avg_complexity||0)/10*.4+(f.dep_count||0)/a*.2}const g=[...e].sort((f,y)=>_(f)-_(y)),E=o/2,S=s/2,w=[];let H=0,ie=0;for(const f of g){const y=c+Math.sqrt(_(f))*(l-c),v=b(f);let $=!1;for(let p=0;p<800;p++){const m=E+ie*Math.cos(H),h=S+ie*Math.sin(H);let x=!1;for(const k of w){const C=m-k.x,z=h-k.y;if(Math.sqrt(C*C+z*z)<y+k.r+2){x=!0;break}}if(!x&&m>y+2&&m<o-y-2&&h>y+25&&h<s-y-2){w.push({x:m,y:h,vx:0,vy:0,r:y,color:v,f}),$=!0;break}H+=.2,ie+=.04}$||w.push({x:E+(Math.random()-.5)*o*.3,y:S+(Math.random()-.5)*s*.3,vx:0,vy:0,r:y,color:v,f})}const Je=[];function lt(f){const y=f.replace(/\\/g,"/").split("/").pop()||"",v=y.lastIndexOf(".");return(v>0?y.substring(0,v):y).toLowerCase()}const We={};w.forEach((f,y)=>{We[lt(f.f.path)]=y});for(const[f,y]of Object.entries(n)){let v=null;if(w.forEach(($,p)=>{$.f.path===f&&(v=p)}),v!==null)for(const $ of y){const p=$.replace(/^\.\//,"").replace(/^\.\.\//,"").split(/[./]/);let m;for(let h=p.length-1;h>=0;h--){const x=p[h].toLowerCase();if(x&&x!=="js"&&x!=="py"&&x!=="rb"&&x!=="ts"&&x!=="index"&&(m=We[x],m!==void 0))break}m===void 0&&(m=We[lt($)]),m!==void 0&&v!==m&&Je.push([v,m])}}const M=document.createElement("canvas");M.width=o,M.height=s,M.style.cssText="display:block;border:1px solid var(--border);border-radius:8px;cursor:pointer;background:#0f172a";const En=i==="framework"?'<span style="color:#cba6f7;font-weight:600">(Framework)</span> Add code to src/ to see your project':"";t.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.5rem"><h3 style="margin:0;font-size:0.85rem">Code Landscape ${En}</h3><span style="font-size:0.65rem;color:var(--muted)">Drag bubbles | Dbl-click to drill down</span></div><div style="position:relative" id="metrics-canvas-wrap"></div>`,document.getElementById("metrics-canvas-wrap").appendChild(M);const Qe=document.createElement("div");Qe.style.cssText="position:absolute;top:8px;left:8px;z-index:2;display:flex;gap:4px;flex-direction:column",Qe.innerHTML=`
  ...
  `,document.getElementById("metrics-canvas-wrap").appendChild(Qe),(yt=document.getElementById("metrics-zoom-in"))==null||yt.addEventListener("click",()=>{L=Math.min(5,L*1.3)}),(ht=document.getElementById("metrics-zoom-out"))==null||ht.addEventListener("click",()=>{L=Math.max(.3,L*.7)}),(ft=document.getElementById("metrics-zoom-fit"))==null||ft.addEventListener("click",()=>{L=1,K=0,X=0});const u=M.getContext("2d");let G=-1,q=-1,dt=0,ct=0,K=0,X=0,L=1,se=!1,mt=0,ut=0,pt=0,gt=0;function Sn(){for(let p=0;p<w.length;p++){if(p===q)continue;const m=w[p],h=E-m.x,x=S-m.y,k=.3+m.r/l*.7,C=.008*k*k;m.vx+=h*C,m.vy+=x*C}for(const[p,m]of Je){const h=w[p],x=w[m],k=x.x-h.x,C=x.y-h.y,z=Math.sqrt(k*k+C*C)||1,P=h.r+x.r+20,N=(z-P)*.002,re=k/z*N,ae=C/z*N;p!==q&&(h.vx+=re,h.vy+=ae),m!==q&&(x.vx-=re,x.vy-=ae)}for(let p=0;p<w.length;p++)for(let m=p+1;m<w.length;m++){const h=w[p],x=w[m],k=x.x-h.x,C=x.y-h.y,z=Math.sqrt(k*k+C*C)||1,P=h.r+x.r+20;if(z<P){const N=40*(P-z)/P,re=k/z*N,ae=C/z*N;p!==q&&(h.vx-=re,h.vy-=ae),m!==q&&(x.vx+=re,x.vy+=ae)}}for(let p=0;p<w.length;p++){if(p===q)continue;const m=w[p];m.vx*=.65,m.vy*=.65;const h=2;m.vx=Math.max(-h,Math.min(h,m.vx)),m.vy=Math.max(-h,Math.min(h,m.vy)),m.x+=m.vx,m.y+=m.vy,m.x=Math.max(m.r+2,Math.min(o-m.r-2,m.x)),m.y=Math.max(m.r+25,Math.min(s-m.r-2,m.y))}}function bt(){var f;Sn(),u.clearRect(0,0,o,s),u.save(),u.translate(K,X),u.scale(L,L),u.strokeStyle="rgba(255,255,255,0.03)",u.lineWidth=1/L;for(let y=0;y<o/L;y+=50)u.beginPath(),u.moveTo(y,0),u.lineTo(y,s/L),u.stroke();for(let y=0;y<s/L;y+=50)u.beginPath(),u.moveTo(0,y),u.lineTo(o/L,y),u.stroke();for(const[y,v]of Je){const $=w[y],p=w[v],m=p.x-$.x,h=p.y-$.y,x=Math.sqrt(m*m+h*h)||1,k=G===y||G===v;u.beginPath(),u.moveTo($.x+m/x*$.r,$.y+h/x*$.r);const C=p.x-m/x*p.r,z=p.y-h/x*p.r;u.lineTo(C,z),u.strokeStyle=k?"rgba(139,180,250,0.9)":"rgba(255,255,255,0.15)",u.lineWidth=k?3:1,u.stroke();const P=k?12:6,N=Math.atan2(h,m);u.beginPath(),u.moveTo(C,z),u.lineTo(C-P*Math.cos(N-.4),z-P*Math.sin(N-.4)),u.lineTo(C-P*Math.cos(N+.4),z-P*Math.sin(N+.4)),u.closePath(),u.fillStyle=u.strokeStyle,u.fill()}for(let y=0;y<w.length;y++){const v=w[y],$=y===G,p=$?v.r+4:v.r;$&&(u.beginPath(),u.arc(v.x,v.y,p+8,0,Math.PI*2),u.fillStyle="rgba(255,255,255,0.08)",u.fill()),u.beginPath(),u.arc(v.x,v.y,p,0,Math.PI*2),u.fillStyle=v.color,u.globalAlpha=$?1:.85,u.fill(),u.globalAlpha=1,u.strokeStyle=$?"rgba(255,255,255,0.6)":"rgba(255,255,255,0.25)",u.lineWidth=$?2.5:1.5,u.stroke();const m=((f=v.f.path.split("/").pop())==null?void 0:f.replace(/\.\w+$/,""))||"?";if(p>16){const k=Math.max(8,Math.min(13,p*.38));u.fillStyle="#fff",u.font=`600 ${k}px monospace`,u.textAlign="center",u.fillText(m,v.x,v.y-2),u.fillStyle="rgba(255,255,255,0.65)",u.font=`${k-1}px monospace`,u.fillText(`${v.f.loc} LOC`,v.x,v.y+k)}const h=Math.max(9,p*.3),x=h*.7;if(p>14&&v.f.dep_count>0){const k=v.y-p+x+3;u.beginPath(),u.arc(v.x,k,x,0,Math.PI*2),u.fillStyle="#ea580c",u.fill(),u.fillStyle="#fff",u.font=`bold ${h}px sans-serif`,u.textAlign="center",u.fillText("D",v.x,k+h*.35)}if(p>14&&v.f.has_tests){const k=v.y+p-x-3;u.beginPath(),u.arc(v.x,k,x,0,Math.PI*2),u.fillStyle="#16a34a",u.fill(),u.fillStyle="#fff",u.font=`bold ${h}px sans-serif`,u.textAlign="center",u.fillText("T",v.x,k+h*.35)}}u.restore(),requestAnimationFrame(bt)}M.addEventListener("mousemove",f=>{const y=M.getBoundingClientRect(),v=(f.clientX-y.left-K)/L,$=(f.clientY-y.top-X)/L;if(se){K=pt+(f.clientX-mt),X=gt+(f.clientY-ut);return}if(q>=0){_e=!0,w[q].x=v+dt,w[q].y=$+ct,w[q].vx=0,w[q].vy=0;return}G=-1;for(let p=w.length-1;p>=0;p--){const m=w[p],h=v-m.x,x=$-m.y;if(Math.sqrt(h*h+x*x)<m.r+4){G=p;break}}M.style.cursor=G>=0?"grab":"default"}),M.addEventListener("mousedown",f=>{const y=M.getBoundingClientRect(),v=(f.clientX-y.left-K)/L,$=(f.clientY-y.top-X)/L;if(f.button===2){se=!0,mt=f.clientX,ut=f.clientY,pt=K,gt=X,M.style.cursor="move";return}G>=0&&(q=G,dt=w[q].x-v,ct=w[q].y-$,_e=!1,M.style.cursor="grabbing")});let _e=!1;M.addEventListener("mouseup",f=>{if(se){se=!1,M.style.cursor="default";return}if(q>=0){_e||Se(w[q].f.path),M.style.cursor="grab",q=-1,_e=!1;return}}),M.addEventListener("mouseleave",()=>{G=-1,q=-1,se=!1}),M.addEventListener("dblclick",f=>{const y=M.getBoundingClientRect(),v=(f.clientX-y.left-K)/L,$=(f.clientY-y.top-X)/L;for(let p=w.length-1;p>=0;p--){const m=w[p],h=v-m.x,x=$-m.y;if(Math.sqrt(h*h+x*x)<m.r+4){Se(m.f.path);break}}}),M.addEventListener("contextmenu",f=>f.preventDefault()),requestAnimationFrame(bt)}async function Se(e){const t=document.getElementById("metrics-detail");if(!t)return;t.innerHTML='<p class="text-muted">Loading file analysis...</p>';const n=await T("/metrics/file?path="+encodeURIComponent(e));if(n.error){t.innerHTML=`<p style="color:var(--danger)">${r(n.error)}</p>`;return}const i=n.functions||[],o=Math.max(1,...i.map(s=>s.complexity));t.innerHTML=`
  ...
  `,Ze()}async function Ze(){const e=document.getElementById("gql-types"),t=document.getElementById("gql-queries"),n=document.getElementById("gql-mutations");try{const i=await T("/graphql/schema");if(i.error){e&&(e.innerHTML=`<p class="text-sm" style="color:var(--danger)">${r(i.error)}</p>`);return}const o=i.schema||{},s=o.types||{},d=o.queries||{},a=o.mutations||{};if(e){const c=Object.keys(s);c.length?e.innerHTML=c.map(l=>{const b=s[l],_=Object.entries(b).map(([g,E])=>`<div style="padding-left:1rem;color:var(--muted);font-size:0.7rem">${r(g)}: <span style="color:var(--primary)">${r(String(E))}</span></div>`).join("");return`
            ...
            </div>`}).join(""):e.innerHTML='<p class="text-sm text-muted">No types registered</p>'}if(t){const c=Object.keys(d);c.length&&(t.innerHTML='<div style="font-weight:600;font-size:0.75rem;color:var(--muted);text-transform:uppercase;margin-bottom:0.25rem">Queries</div>'+c.map(l=>{const b=d[l],_=b.args?Object.entries(b.args).map(([g,E])=>`${g}: ${E}`).join(", "):"";return`<div style="font-size:0.8rem;padding:0.15rem 0;cursor:pointer;color:var(--text)" onclick="window.__insertGqlQuery('${r(l)}','query')" title="Click to insert">${r(l)}${_?`(${r(_)})`:""}: <span style="color:var(--primary)">${r(b.type||"")}</span></div>`}).join(""))}if(n){const c=Object.keys(a);c.length&&(n.innerHTML='<div style="font-weight:600;font-size:0.75rem;color:var(--muted);text-transform:uppercase;margin-bottom:0.25rem">Mutations</div>'+c.map(l=>{const b=a[l],_=b.args?Object.entries(b.args).map(([g,E])=>`${g}: ${E}`).join(", "):"";return`<div style="font-size:0.8rem;padding:0.15rem 0;cursor:pointer;color:var(--text)" onclick="window.__insertGqlQuery('${r(l)}','mutation')" title="Click to insert">${r(l)}${_?`(${r(_)})`:""}: <span style="color:var(--primary)">${r(b.type||"")}</span></div>`}).join(""))}}catch(i){e&&(e.innerHTML=`<p class="text-sm" style="color:var(--danger)">${r(i.message)}</p>`)}}function Jt(e,t){const n=document.getElementById("gql-query");n&&(t==="mutation"?n.value=`mutation {
  ${e} {
...
`);n&&(n.style.display="block",n.textContent=b)}i&&(i.textContent=JSON.stringify(l.data??l,null,2))}catch(l){n&&(n.style.display="block",n.textContent=l.message),i&&(i.textContent="")}}function Qt(){Te&&navigator.clipboard.writeText(JSON.stringify(Te,null,2))}window.__loadGqlSchema=Ze,window.__runGqlQuery=Wt,window.__copyGqlResult=Qt,window.__insertGqlQuery=Jt;let Ie=!1,Me=null,de="jobs",V="",R="default",ce=["default"],me=null;function Ut(e){me=e,T("/queue/topics").then(t=>{t.topics&&t.topics.length>0&&(ce=t.topics,ce.includes(R)||(R=ce[0])),qe()}).catch(()=>qe())}function qe(){if(!me)return;const e=ce.map(t=>`<option value="${r(t)}" ${t===R?"selected":""}>${r(t)}</option>`).join("");me.innerHTML=`
        ...
        <tbody>
          ${t.jobs.map(s=>`
            <tr>
              ...
              <td><code style="font-size:0.7rem;word-break:break-all;max-width:300px;display:inline-block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer" title="${r(JSON.stringify(s.data||{},null,2))}" onclick="alert(this.title)">${r(JSON.stringify(s.data||{}).slice(0,80))}</code></td>
        ...
        <tbody>
          ${e.jobs.map(o=>`
            <tr>
              ...
              <td><code style="font-size:0.7rem;word-break:break-all;max-width:250px;display:inline-block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer" title="${r(JSON.stringify(o.data||{},null,2))}" onclick="alert(this.title)">${r(JSON.stringify(o.data||{}).slice(0,60))}</code></td>
  ...
  `}let be=0;function B(e,t){var s,d;const n=document.getElementById("chat-messages");if(!n)return;const i=`msg-${++be}`,o=document.createElement("div");if(o.className=`chat-msg chat-${t}`,o.id=i,o.innerHTML=`

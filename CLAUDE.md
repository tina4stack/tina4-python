...
**Before doing anything in this repo, invoke the `tina4-maintainer` skill** (Skill tool: `tina4-maintainer`, or the directory-scoped variant `tina4-python:tina4-maintainer`). For UI/frontend work also load `tina4-js`. Re-invoke after any context compaction.
...
Version 3.13.134 - Lightweight Python web framework. See https://tina4.com for full documentation.
...
**One client per URI, and a way to close it.** `get_collection` caches the `MongoClient` rather than building a new one per call — before 3.13.95 it constructed one on EVERY call and never closed it, so 20 calls left ~39 server connections open and the count grew without bound (invisible locally, because the SQLite fallback opens no connections at all). `close_doc_store()` closes every Mongo client and the SQLite store.
...
- CLI scaffolding: `tina4 generate model/route/migration/middleware`. `tina4python` is the Python package's own CLI; the `tina4` client forwards `generate` to it.

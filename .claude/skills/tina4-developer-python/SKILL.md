...
Climb in order; write new code only at the last rung. Tina4 ships **98 built-in features, zero dependencies** — most "new code" is already in the box, and most of the rest can be **scaffolded**.
...
1. **Embellish with Tina4 principles** — restate the ask through the reuse ladder, convention
   ...
   zero pip deps. Example: "add auth" → "public `POST /api/login` mints JWT via

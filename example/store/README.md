...
| http://localhost:7145/api/soap/orders?wsdl | WSDL definition |
| http://localhost:7145/__dev/ | DevAdmin metrics |
...
```python
customer.delete()                    # Sets deleted_at (not visible in queries)
customer.force_delete()              # Actually removes the row
customer.restore()                   # Clears deleted_at

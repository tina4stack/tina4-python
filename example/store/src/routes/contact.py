...
def _build_contact_form(success=False, error=None):
    ...
    form = _form({"method": "POST", "action": "/contact", "class": "contact-form"},
        _div({"class": "form-group mb-3"},
            ...
            _input({"type": "text", "id": "name", "name": "name", "class": "form-control", "required": "required"}),
        ),
        _div({"class": "form-group mb-3"},
            _label({"for": "email"}, "Email"),
            _input({"type": "email", "id": "email", "name": "email", "class": "form-control", "required": "required"}),
        ),
        _div({"class": "form-group mb-3"},
            _label({"for": "subject"}, "Subject"),
            _input({"type": "text", "id": "subject", "name": "subject", "class": "form-control"}),
        ),
        _div({"class": "form-group mb-3"},
            _label({"for": "message"}, "Message"),
            _textarea({"id": "message", "name": "message", "class": "form-control", "rows": "5", "required": "required"}),
        ),
